...
if ( $save_results != '' ) {
      ...
      if ( $home_val != '' && $away_val != '' ) {
             save_result($game_id, abs(intval($home_val)), abs(intval($away_val)));
      }
...
for ( $i = 0; $i < count($games_data); $i++ ) {
    ...
    $template->assign_block_vars('grouprow', array(
		        'GROUP_NAME'   => $lang['l_em_round1_group'] . '&nbsp;' . $games_row[0]['game_group'],
...
//               $em_tipps_pts .= '<img src="./images/em/teamgeist.gif" alt="Teamgeist" border="0" />';
        ...
        $template->assign_block_vars('grouprow.gamesrow', array(
			...
			'GAME_AWAY_TEAM'      => $teams_data[$games_row[$j]['game_away']]['team_name'],
			'GAME_AWAY_IMG'       => $teams_data[$games_row[$j]['game_away']]['team_img'],
			'GAME_AWAY_LINK'      => $teams_data[$games_row[$j]['game_away']]['team_link'],
			'GAME_HOME_TEAM'      => $teams_data[$games_row[$j]['game_home']]['team_name'],
			'GAME_HOME_IMG'       => $teams_data[$games_row[$j]['game_home']]['team_img'],
			'GAME_HOME_LINK'      => $teams_data[$games_row[$j]['game_home']]['team_link']
             )
        );
    }
}
...
$template->assign_vars(array(
                             ...
                             'S_FORM_ACTION'          => append_sid("./em_round1.".$phpEx)
        )
);
