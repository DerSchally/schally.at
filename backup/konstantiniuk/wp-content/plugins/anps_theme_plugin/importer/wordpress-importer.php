...
class WP_Import extends WP_Importer { 
	...
	function import_end() {
		...
		echo '<h2 class="import-success-title">' . esc_html__( 'All done.', 'wordpress-importer' ) . ' <a href="' . admin_url() . '">' . esc_html__( 'Have fun!', 'wordpress-importer' ) . '</a>' . '</h2>';
		echo '<p class="import-success-subtitle">' . esc_html__( 'Remember to update the passwords and roles of imported users.', 'wordpress-importer' ) . '</p>';
	...
	function get_authors_from_import( $import_data ) {
		...
		// no author information, grab it from the posts
			...
			foreach ( $import_data['posts'] as $post ) {
				...
				if ( empty( $login ) ) {
					printf( esc_html__( 'Loading', 'wordpress-importer' ), esc_html( $post['post_author'] ) );
					echo '<br />';
					continue;
				}
